<?php
    ...
    if ($id == "") {
        $query = "INSERT INTO `datasiswanya` (`nama_siswa`, `alamat`, `jurusan`, `jenis_kelamin`, `tgl_masuk`) VALUES (?, ?, ?, ?, ?)";
        $prepare1 = $db1->prepare($query);
        $prepare1->bind_param("sssss", $nama_siswa, $alamat, $jurusan, $jkl, $tgl_masuk);
        $prepare1->execute();
        echo $query;
    } else {
        $query = "UPDATE `datasiswanya` SET `nama_siswa` = ?, `alamat` = ?, `jurusan` = ?, `jenis_kelamin` = ?, `tgl_masuk` =? WHERE `datasiswanya`.`id` = ?";
        $prepare1 = $db1->prepare($query);
